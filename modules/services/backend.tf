resource "aws_launch_template" "MoviesBackEndTemplate" {
  ...
  vpc_security_group_ids = ["${module.stage.network.SG_BE_EC2_id}"]
  ...
  user_data = base64encode(<<-EOF
              ...
              echo 'DB_HOST="${module.stage.stage_database.moviesDB_address}"' >> /etc/environment
              echo 'DB_USER="${module.stage.stage_database.moviesDB_username}"' >> /etc/environment
              echo 'DB_PASS="${module.stage.stage_database.moviesDB_password}"' >> /etc/environment
...
resource "aws_autoscaling_group" "MoviesBackEndAS" {
  ...
  vpc_zone_identifier = [module.stage.stage_network.subnet_PriBE1_id, module.stage.stage_network.subnet_PriBE2_id]
...
resource "aws_lb" "MoviesLBBackEnd" {
  ...
  subnets            = [module.stage.stage_network.subnet_PriBE1_id, module.stage.stage_network.subnet_PriBE2_id]
  security_groups    = [module.stage.stage_network.SG_LB_INT_BE_id]
}
